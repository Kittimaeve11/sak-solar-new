...
export default function AboutPage() {
  ...
  useEffect(() => {
    ...
    document.title = 'เกี่ยวกับเรา | บริษัท ศักดิ์สยาม โซลาร์ เอ็นเนอร์ยี่ จำกัด';
    const metaDescription = document.querySelector("meta[name='description']");
    if (metaDescription) {
      metaDescription.setAttribute("about", "เกี่ยวกับเรา");
    } else {
      const meta = document.createElement('meta');
      meta.name = 'description';
      meta.content = 'หน้าเกี่ยวกับเรา';
      document.head.appendChild(meta);
    }
    // --------- 1. IntersectionObserver สำหรับ active menu ตาม scroll ----------
    if (selectedMenu !== 'teams') {
    ...
    // --------- 2. Scroll ไปยัง pendingScrollId ----------
    if (pendingScrollId) {
      ...
      if (!scrollToElement(pendingScrollId)) {
  ...
  }, [loading, selectedMenu, pendingScrollId, pathname]);
