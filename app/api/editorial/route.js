export async function GET() {
  const editorial = [
    {
      "id": 1,
      "title": "การบำรุงรักษาโซลาร์เซลล์อย่างถูกต้อง",
      "type": "การดูแลรักษา",
      "date": "26 มิถุนายน 2568",
      "mainImage": "/editorial/editorial1/editorial-main-1.jpg",
      "content": [
        {
          "type": "h1",
          "text": "การบำรุงรักษาโซลาร์เซลล์อย่างถูกต้อง"
        },
        {
          "type": "paragraph",
          "text": "ระบบโซลาร์เซลล์ถือเป็นหนึ่งในพลังงานสะอาดที่ได้รับความนิยมมากขึ้นในปัจจุบัน การดูแลรักษาที่เหมาะสมไม่เพียงแต่ช่วยยืดอายุการใช้งานของระบบ แต่ยังช่วยให้คุณได้รับพลังงานที่มีประสิทธิภาพสูงสุดจากการลงทุนในระยะยาว"
        },
        {
          "type": "paragraph",
          "text": "บทความนี้จะกล่าวถึงแนวทางสำคัญในการบำรุงรักษาโซลาร์เซลล์ ทั้งในเรื่องความถี่ของการดูแล การตรวจสอบอุปกรณ์ต่าง ๆ และข้อควรระวังที่สำคัญ"
        },
        {
          "type": "h2",
          "text": "ความถี่ในการทำความสะอาด"
        },
        {
          "type": "paragraph",
          "text": "แผงโซลาร์เซลล์ควรทำความสะอาดอย่างน้อยทุก 3–6 เดือน เพื่อป้องกันการสะสมของฝุ่นละออง เศษใบไม้ หรือมูลนก ซึ่งล้วนแล้วแต่ลดทอนประสิทธิภาพการรับแสงของแผง"
        },
        {
          "type": "paragraph",
          "text": "ในพื้นที่ที่มีฝุ่นมาก เช่น ใกล้ถนนใหญ่ โรงงาน หรือพื้นที่ก่อสร้าง ควรตรวจสอบและทำความสะอาดบ่อยขึ้น เช่น เดือนละ 1 ครั้ง เพื่อรักษาระดับประสิทธิภาพให้สูงที่สุด"
        },
        {
          "type": "paragraph",
          "text": "ควรทำความสะอาดในช่วงเช้าหรือเย็น เมื่อแผงไม่ร้อนจัด เพื่อหลีกเลี่ยงความเสียหายที่อาจเกิดจากอุณหภูมิสูงผสมกับน้ำ"
        },
        {
          "type": "h2",
          "text": "การตรวจสอบระบบไฟฟ้า"
        },
        {
          "type": "paragraph",
          "text": "การตรวจสอบระบบไฟฟ้าเป็นประจำทุกเดือนเป็นสิ่งจำเป็น โดยควรดูข้อมูลการผลิตไฟฟ้าผ่านแอปพลิเคชันหรือหน้าจอแสดงผลของระบบ เพื่อดูว่าแผงยังผลิตพลังงานได้ในระดับปกติหรือไม่"
        },
        {
          "type": "h3",
          "text": "2.1 อินเวอร์เตอร์และสัญญาณไฟ"
        },
        {
          "type": "paragraph",
          "text": "อินเวอร์เตอร์เป็นอุปกรณ์สำคัญที่แปลงพลังงานไฟฟ้าจากกระแสตรง (DC) ไปเป็นกระแสสลับ (AC) หากไฟแสดงสถานะขึ้นเป็นสีเขียว หมายถึงระบบทำงานปกติ"
        },
        {
          "type": "paragraph",
          "text": "แต่หากพบว่ามีไฟสีแดง หรือไม่มีไฟแสดงผลเลย นั่นอาจบ่งบอกถึงปัญหาที่ควรให้ช่างผู้เชี่ยวชาญเข้ามาตรวจสอบทันที เพื่อป้องกันความเสียหายต่อระบบ"
        },
        {
          "type": "h3",
          "text": "2.2 ตรวจสอบอุปกรณ์ประกอบ"
        },
        {
          "type": "paragraph",
          "text": "ควรตรวจสอบสายไฟ จุดเชื่อมต่อ และเบรกเกอร์อย่างสม่ำเสมอ เพื่อป้องกันปัญหาไฟรั่วหรือการเสื่อมสภาพของอุปกรณ์ที่อาจทำให้ระบบล้มเหลว"
        },
        {
          "type": "paragraph",
          "text": "การตรวจสอบควรทำโดยบุคลากรที่ได้รับการฝึกอบรม เพราะการสัมผัสอุปกรณ์โดยตรงอาจเกิดอันตรายได้หากไม่มีความรู้"
        },
        {
          "type": "h2",
          "text": "ข้อควรระวังในการดูแลรักษา"
        },
        {
          "type": "paragraph",
          "text": "ก่อนทำความสะอาดหรือซ่อมแซมแผง ควรปิดระบบทั้งหมด โดยเฉพาะอินเวอร์เตอร์และเบรกเกอร์ เพื่อความปลอดภัยในการทำงาน"
        },
        {
          "type": "paragraph",
          "text": "ควรหลีกเลี่ยงการใช้แรงดันน้ำสูงหรือสารเคมีรุนแรง เพราะอาจทำลายผิวแผง หรือก่อให้เกิดคราบเคมีตกค้างที่ลดทอนประสิทธิภาพ"
        },
        {
          "type": "paragraph",
          "text": "การใช้อุปกรณ์ทำความสะอาดที่เหมาะสม เช่น ผ้านุ่ม แปรงขนนุ่ม และน้ำสะอาด จะช่วยให้การดูแลแผงมีประสิทธิภาพโดยไม่ก่อให้เกิดความเสียหาย"
        },
        {
          "type": "h3",
          "text": "3.1 หลีกเลี่ยงการเหยียบหรือกดทับแผง"
        },
        {
          "type": "paragraph",
          "text": "การเหยียบหรือกดทับบนแผงโซลาร์เซลล์อาจทำให้แผงแตกภายในโดยที่ไม่เห็นความเสียหายภายนอก และทำให้ประสิทธิภาพลดลงอย่างมาก"
        },
        {
          "type": "h3",
          "text": "3.2 การเลือกบริษัทดูแลระบบ"
        },
        {
          "type": "paragraph",
          "text": "หากคุณไม่มีความเชี่ยวชาญหรือไม่มีเวลาทำเอง ควรเลือกบริษัทที่มีใบรับรองและประสบการณ์ในการดูแลระบบโซลาร์ โดยตรวจสอบประวัติงานและรีวิวจากลูกค้าเดิม"
        },
        {
          "type": "paragraph",
          "text": "บริการจากมืออาชีพจะช่วยตรวจสอบได้ลึกถึงระดับเซลล์ของแผง และสามารถวิเคราะห์ปัญหาที่อาจเกิดในอนาคตได้อย่างแม่นยำ"
        },
        {
          "type": "h2",
          "text": "สรุปการดูแลรักษาโซลาร์เซลล์"
        },
        {
          "type": "paragraph",
          "text": "การดูแลรักษาโซลาร์เซลล์ไม่จำเป็นต้องทำทุกวัน แต่ควรมีความสม่ำเสมอและรอบคอบ เพื่อให้ได้ผลตอบแทนสูงสุดจากระบบพลังงานแสงอาทิตย์ที่คุณลงทุนไว้"
        },
        {
          "type": "paragraph",
          "text": "หากคุณใส่ใจในรายละเอียดและมีวินัยในการตรวจสอบเป็นประจำ ระบบโซลาร์จะสามารถอยู่กับคุณได้ยาวนานนับสิบปี โดยให้ผลลัพธ์ที่คุ้มค่าอย่างแท้จริง"
        }

      ],
      "gallery": [
        "/editorial/editorial1/editorial-gallery-1.jpg",
        "/editorial/editorial1/editorial-gallery-2.jpg",
        "/editorial/editorial1/editorial-gallery-3.jpg",
        "/editorial/editorial1/editorial-gallery-4.jpg",
        "/editorial/editorial1/editorial-gallery-5.jpg"
      ]
    },
    {
      id: 2,
      title: "เทรนด์เทคโนโลยีโซลาร์เซลล์ในปี 2024",
      type: "โซลาร์เซลล์",
      date: "26 มิถุนายน 2568",
      mainImage: "/editorial/beautiful-alternative-energy-plant-with-solar-panels_9_11zon.jpg",
      content: `ปี 2024 เทคโนโลยีโซลาร์เซลล์มีการพัฒนาอย่างต่อเนื่อง เช่น แผงแบบ PERC และ TOPCon ที่ให้ประสิทธิภาพสูงขึ้น นอกจากนี้ยังมีการเชื่อมต่อกับระบบสมาร์ทโฮมที่สามารถควบคุมผ่านแอปพลิเคชัน และการใช้งานแบตเตอรี่ลิเธียมไอออนที่ปลอดภัยและเก็บพลังงานได้ยาวนาน`,
      gallery: ["/editorial/article4-1.jpg", "/editorial/article4-2.jpg"]
    },
    {
      id: 3,
      title: "การคำนวณผลตอบแทนจากการติดตั้งโซลาร์",
      type: "โซลาร์เซลล์",
      date: "26 มิถุนายน 2568",
      mainImage: "/editorial/solar-panels-red-tile-roof-with-clear-blue-sky-background_21_11zon.jpg",
      content: `การคำนวณ ROI ก่อนติดตั้งระบบโซลาร์เซลล์ควรพิจารณาค่าไฟเฉลี่ยต่อเดือน ขนาดระบบที่เหมาะสม ราคาติดตั้ง และประสิทธิภาพของระบบ โดยทั่วไป การลงทุนในโซลาร์เซลล์สามารถคืนทุนได้ภายใน 5–7 ปี ขึ้นอยู่กับปัจจัยด้านการใช้ไฟและราคาพลังงานในอนาคต`,
      gallery: ["/editorial/article5-1.jpg", "/editorial/article5-2.jpg"]
    },
    {
      id: 4,
      title: "แผงโซลาร์เซลล์แบบไหนเหมาะกับเมืองไทย",
      type: "โซลาร์เซลล์",
      date: "26 มิถุนายน 2568",
      mainImage: "/editorial/bernd-dittrich-CDA62CsGDDM-unsplash_10_11zon.jpg",
      content: `แผง Monocrystalline เหมาะกับประเทศไทยเนื่องจากมีแดดจัดและให้ประสิทธิภาพสูงกว่าประเภทอื่น ควรเลือกแผงที่ได้รับการรับรองมาตรฐาน มอก. และมีการรับประกันอย่างน้อย 25 ปี รวมถึงเลือกผู้ผลิตที่มีศูนย์บริการในประเทศเพื่อความมั่นใจในการใช้งานระยะยาว`,
      gallery: ["/editorial/article6-1.jpg", "/editorial/article6-2.jpg"]
    },
    {
      id: 5,
      title: "โซลาร์เซลล์กับระบบกักเก็บพลังงาน (Battery)",
      type: "แบตเตอรี่",
      date: "26 มิถุนายน 2568",
      mainImage: "/editorial/bill-mead-wmaP3Tl80ww-unsplash_11_11zon.jpg",
      content: `ระบบแบตเตอรี่ช่วยให้สามารถเก็บพลังงานไว้ใช้ในช่วงกลางคืนหรือเมื่อไฟดับ โดยเฉพาะในพื้นที่ที่ไฟฟ้าไม่เสถียร ปัจจุบันนิยมใช้แบตเตอรี่ลิเธียมไอออนที่มีความปลอดภัยและอายุการใช้งานยาวนาน เหมาะสำหรับบ้านที่ต้องการพึ่งพาพลังงานแสงอาทิตย์แบบ 100%`,
      gallery: ["/editorial/article7-1.jpg", "/editorial/article7-2.jpg"]
    },
    {
      id: 6,
      title: "ระบบ Net Metering และการขายไฟกลับ",
      type: "Net Metering",
      date: "26 มิถุนายน 2568",
      mainImage: "/editorial/close-view-solar-panel_12_11zon.jpg",
      content: `ระบบ Net Metering ช่วยให้เจ้าของบ้านสามารถขายไฟฟ้าส่วนเกินกลับเข้าสู่ระบบของการไฟฟ้า ซึ่งขึ้นอยู่กับขนาดของระบบและนโยบายของรัฐ การติดตั้งระบบนี้ต้องขออนุญาตจากหน่วยงานที่เกี่ยวข้อง และควรศึกษาข้อมูลให้ชัดเจนก่อนดำเนินการ`,
      gallery: ["/editorial/article8-1.jpg", "/editorial/article8-2.jpg"]
    },
    {
      id: 7,
      title: "ค่าใช้จ่ายในการดูแลระบบโซลาร์ระยะยาว",
      type: "ค่าใช้จ่าย",
      date: "26 มิถุนายน 2568",
      mainImage: "/editorial/markus-spiske-rNn_TU8dvoY-unsplash_13_11zon.jpg",
      content: `แม้ว่าโซลาร์เซลล์ไม่มีค่าใช้จ่ายรายเดือน แต่ควรเผื่องบประมาณในการบำรุงรักษา เช่น ค่าทำความสะอาด ตรวจสอบระบบ และเปลี่ยนอินเวอร์เตอร์เมื่อครบอายุการใช้งาน ประมาณการค่าใช้จ่ายอยู่ที่ 1–2% ของค่าติดตั้งต่อปี`,
      gallery: ["/editorial/article9-1.jpg", "/editorial/article9-2.jpg"]
    },
    {
      id: 8,
      title: "ประโยชน์ของการติดตั้งโซลาร์เซลล์ (อัปเดต)",
      type: "โซลาร์เซลล์",
      date: "26 มิถุนายน 2568",
      mainImage: "/editorial/modern-solar-power-station-generates-sustainable-electricity-outdoors-generated-by-ai_14_11zon.jpg",
      content: `โซลาร์เซลล์มีข้อดีทั้งด้านสิ่งแวดล้อมและเศรษฐกิจ เช่น การลดคาร์บอน ลดค่าไฟ และเพิ่มความสามารถในการพึ่งพาตนเองด้านพลังงาน อีกทั้งยังส่งเสริมการใช้พลังงานสะอาดในระดับประเทศอย่างยั่งยืน`,
      gallery: ["/editorial/article10-1.jpg", "/editorial/article10-2.jpg"]
    },
    {
      id: 9,
      title: "การติดตั้งโซลาร์เซลล์บนหลังคาเชิงพาณิชย์",
      type: "โซลาร์เซลล์",
      date: "1 กรกฎาคม 2568",
      mainImage: "/editorial/aerial-view-private-house-with-solar-panels-roof_7_11zon.jpg",
      content: `การติดตั้งโซลาร์เซลล์บนหลังคาอาคารพาณิชย์สามารถลดค่าไฟฟ้าของธุรกิจได้อย่างมีนัยสำคัญ การวางแผนโครงสร้างหลังคาและระบบไฟฟ้าอย่างเหมาะสม รวมถึงเลือกใช้อุปกรณ์ที่มีคุณภาพ จะช่วยให้การลงทุนคุ้มค่าในระยะยาว`,
      gallery: ["/editorial/commercial-1.jpg", "/editorial/commercial-2.jpg"]
    },
    {
      id: 10,
      title: "การติดตั้งโซลาร์เซลล์บนหลังคาเชิงพาณิชย์",
      type: "โซลาร์เซลล์",
      date: "1 กรกฎาคม 2568",
      mainImage: "/editorial/modern-solar-power-station-generates-sustainable-electricity-outdoors-generated-by-ai_14_11zon.jpg",
      content: `การติดตั้งโซลาร์เซลล์บนหลังคาอาคารพาณิชย์สามารถลดค่าไฟฟ้าของธุรกิจได้อย่างมีนัยสำคัญ การวางแผนโครงสร้างหลังคาและระบบไฟฟ้าอย่างเหมาะสม รวมถึงเลือกใช้อุปกรณ์ที่มีคุณภาพ จะช่วยให้การลงทุนคุ้มค่าในระยะยาว`,
      gallery: ["/editorial/commercial-1.jpg", "/editorial/commercial-2.jpg"]
    }
  ];

  return Response.json({ editorial });
}